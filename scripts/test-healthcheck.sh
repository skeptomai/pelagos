...
cleanup() {
    echo ""
    echo "--- Cleanup ---"
    $PELAGOS stop "$NAME" 2>/dev/null || true
    sleep 0.5
    $PELAGOS rm -f "$NAME" 2>/dev/null || true
    $PELAGOS image rm "$IMAGE" 2>/dev/null || true
    echo "Done."
}
trap cleanup EXIT
...
$PELAGOS build -t "$IMAGE" "$CONTEXT"
echo ""

echo "=== Step 3: Run detached ==="
$PELAGOS run -d --name "$NAME" "$IMAGE"
echo ""
