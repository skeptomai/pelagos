...
~/Projects/home-monitoring/remora/
  ...
  config/
    prometheus/prometheus.yml          # Scrape targets + alerting rules
...
# Stream one service:
sudo remora compose logs -f ~/Projects/home-monitoring/remora/compose.rem -p home-monitoring --follow snmp-exporter
...
prometheus). Prometheus uses service names in `prometheus.yml`.
